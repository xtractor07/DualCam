...
import com.example.dualcam.databinding.ActivityMainBinding
import com.example.dualcam.view.ConfirmationDialog
import com.example.dualcam.view.ErrorDialog
import java.util.*
...
class MainActivity : AppCompatActivity() {
     ...
     */
    private lateinit var textureViewRear: TextureView
    ...
    private val secondaryLooperHandler = Handler(Looper.getMainLooper())
    /**
...
//            cameraDeviceFront = cameraDevice
//            try {
//                secondaryLooperHandler.post {
//                    if (textureViewFront.isAvailable) {
//                        createFrontCaptureSession()
//                    }
//                    textureViewFront.surfaceTextureListener = frontSurfaceTextureListener
//                }
//            } catch (t: Throwable) {
...
//        }
            ...
            try {
                mainLooperHandler.post {
                    if (textureViewRear.isAvailable) {
                        createRearCaptureSession()
                    }
                    textureViewRear.surfaceTextureListener = rearSurfaceTextureListener
                }
            } catch (t: Throwable) {
                onDisconnected(cameraDevice)
                Log.e(TAG, "Failed to initialize camera.", t)
    ...
    private fun createRearCaptureSession() {
        if (cameraDeviceRear == null || !textureViewRear.isAvailable) return

        /*** Codelab --> Change this function to accept display rotation ***/
        val targetTexture = CameraUtils.buildTargetTexture(
            textureViewRear, rearManager.getCameraCharacteristics(cameraIdRear)
        )

        this.surface = Surface(targetTexture)
        try {
            cameraDeviceRear?.createCaptureSession(
                listOf(surface), rearSessionStateCallback, cameraHandler
            )
        } catch (t: Throwable) {
    ...
    private fun createFrontCaptureSession() {
        ...
        /*** Codelab --> Change this function to accept display rotation ***/
        val targetTexture = CameraUtils.buildTargetTexture(
            textureViewFront, frontManager.getCameraCharacteristics(cameraIdFront)
        )

        this.surface = Surface(targetTexture)
        try {
            cameraDeviceFront?.createCaptureSession(
                listOf(surface), frontSessionStateCallback, cameraHandler
            )
        } catch (t: Throwable) {
            Log.e(TAG, "Failed to create session.", t)
        }
    ...
    private val rearSessionStateCallback = object : CameraCaptureSession.StateCallback() {
        override fun onConfigured(cameraCaptureSession: CameraCaptureSession) {
            try {
                val captureRequest = cameraDeviceRear?.createCaptureRequest(
    ...
    private val frontSessionStateCallback = object : CameraCaptureSession.StateCallback() {
        override fun onConfigured(cameraCaptureSession: CameraCaptureSession) {
            try {
                val captureRequest = cameraDeviceFront?.createCaptureRequest(
                    CameraDevice.TEMPLATE_PREVIEW
                )
                captureRequest?.addTarget(surface!!)
                cameraCaptureSession.setRepeatingRequest(
    ...
    private val rearSurfaceTextureListener = object : TextureView.SurfaceTextureListener {
        ...
        override fun onSurfaceTextureAvailable(
            ...
            createRearCameraPreviewSession()
        }
    ...
    private val frontSurfaceTextureListener = object : TextureView.SurfaceTextureListener {
        ...
        override fun onSurfaceTextureAvailable(
            ...
            createFrontCameraPreviewSession()
        }
    ...
    private lateinit var rearManager: CameraManager
    private lateinit var frontManager: CameraManager
    private lateinit var binding: ActivityMainBinding

    override fun onCreate(savedInstanceState: Bundle?) {
        rearManager = this.getSystemService(Context.CAMERA_SERVICE) as CameraManager
        frontManager = this.getSystemService(Context.CAMERA_SERVICE) as CameraManager
    ...
    private fun openCameraFront(width: Int, height: Int) {
        val permission = this.let { ContextCompat.checkSelfPermission(it, Manifest.permission.CAMERA) }
...
//        configureTransformRear(width, height)
    ...
    private fun requestCameraPermission() {
        if (shouldShowRequestPermissionRationale(Manifest.permission.CAMERA)) {
            ConfirmationDialog().show(this.supportFragmentManager, FRAGMENT_DIALOG)
        } else {
        ...
        try {
            for (cameraId in manager.cameraIdList) {
                ...
                val aspectRatio = Collections.max(
                    ...
                    imageReaderFront = ImageReader.newInstance(200, 200,
...
//                }
            ...
            ErrorDialog.newInstance(getString(R.string.camera_error))
                .show(this.supportFragmentManager, FRAGMENT_DIALOG)
        }
...
//                }
     ...
     */
    private fun createRearCameraPreviewSession() {
        try {
...
//                        activity.showToast("Failed")
    ...
    private fun createFrontCameraPreviewSession() {
        try {
            val texture = textureViewFront.surfaceTexture
    ...
    companion object {
        ...
        @JvmStatic private fun chooseOptimalSize(
            ...
            return if (bigEnough.size > 0) {
                Collections.min(bigEnough, CompareSizesByViewAspectRatio(textureViewHeight, textureViewWidth))
            } else if (notBigEnough.size > 0) {
                Collections.max(notBigEnough, CompareSizesByViewAspectRatio(textureViewHeight, textureViewWidth))
            } else {
